...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate a video with LTX-2 T2V.")
    parser.add_argument(
        "--model",
        default="Lightricks/LTX-2",
        help="LTX-2 model ID or local path.",
    )
    parser.add_argument(
        "--prompt",
    ...
    parser.add_argument("--guidance_scale", type=float, default=4.0, help="CFG scale.")
    parser.add_argument("--height", type=int, default=512, help="Video height (must be divisible by 32).")
    parser.add_argument("--width", type=int, default=768, help="Video width (must be divisible by 32).")
    parser.add_argument(
        "--num_frames", type=int, default=121, help="Number of frames (should be 8*n+1, e.g., 25, 81, 121)."
    )
    parser.add_argument("--num_inference_steps", type=int, default=40, help="Sampling steps.")
    parser.add_argument("--output", type=str, default="ltx2_output.mp4", help="Path to save the video (mp4).")
